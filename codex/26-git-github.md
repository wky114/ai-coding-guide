...
先甩一组数据。GitHub 官方在《2024 State of the Octoverse》里给过一个口径：一个 PR 从开出来到合进去，**中位等待时间里很大一块耗在「等人 review」上**——不是 review 本身慢，是**根本排不上队**。团队小、reviewer 忙，你的 PR 就在那儿挂着，挂一天是常事。
...
发出去之后会发生什么？按官方文档，节奏是这样的：

1. **Codex 先给你的评论加个 👀 反应**——表示「收到，我开始看了」。
...
> 💡 一句话总结：本地 `/review` 是**开 PR 前的自查门**——在终端选预设（未提交 / 比分支 / 某 commit / 自定义），它**只读不写**地报 P0/P1；先在本地改干净再推，远端 review 就少而精；要更强的审查模型设 `review_model`。
...
**预期**：`git init` 建好仓库，提交成功打印一行 `[main (root-commit) xxxxxxx] feat: 初始 get_user`。**注意我故意在 `app.py` 里埋了个 SQL 注入（字符串拼接 SQL)——一会儿看 Codex 能不能揪出来。**
...
回到开头那组数据——**PR 排队等 review 是常态，人 review 又累又容易漏要命的坑**。把审查交给一个不会累、只报高优先级、还按你规矩挑刺的 Codex，你的 PR 不用再干等；而那条「合并放行」的红线攥在自己手里，你就既快又稳。这，就是 Git 协作里人和 AI 最舒服的分工。
