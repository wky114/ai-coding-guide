...
> ⚠️ **一个 Git 仓库的硬性前提**：Codex 默认**要求命令在一个 Git 仓库里跑**，这是为了防止它在没有版本管理的地方造成不可逆的破坏。要是你在一个非 Git 目录里跑 `codex exec`，它会直接拦下来。临时目录里想跑，加 `--skip-git-repo-check` 跳过这道检查——但官方原话是「**只在你确信环境安全时**」才这么干。

> 💡 一句话总结：`codex exec "一句话"` 就是最基础的非交互跑法（短名 `codex e`），像发一封带任务的邮件等回信——提示词得一次写清，它跑完就退、**中途不停下来问你批不批准**；还有个硬前提：默认得在 Git 仓库里跑，非 Git 目录加 `--skip-git-repo-check`。
