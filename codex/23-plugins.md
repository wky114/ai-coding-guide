...
都说插件嘛，不就是个「打包」的便利功能，可有可无，先把单个功能用熟再说？**说句实话，这判断反了。** 我自己踩过这个亏：今年四月我在一个项目里给 Codex 配了一套「拉数据库迁移、生成 schema 文档」的 skill，加上一个连内部工具的 MCP server，前后弄了俩小时。两周后换到另一个仓库想复用同一套——我对着旧项目的目录一个文件一个文件抄，结果漏了 `.mcp.json` 里一段环境变量配置，新项目里那个工具死活连不上，又排查了半小时。**散件管理，就是这么又累又容易漏。**
...
那它和「单写一个 skill」该怎么选？官方给的判断很干脆：**还在一个仓库、一条个人工作流里来回改，就先用本地 skill；想跨团队共享、想把 app 集成和 MCP 配置一起打包、想发一个稳定的版本化包了，才上插件。**
...
但 Codex 在这儿留了一道我特别想点赞的设计——**hook 不会因为你装了、启用了插件就自动被信任**。官方原话说得明白：

> 安装或启用插件不会自动信任它的 hook。插件打包的 hook 属于非托管（non-managed）hook，所以在用户审阅并信任当前 hook 定义之前，Codex 会跳过它们。
