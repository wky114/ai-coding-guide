...
启用后，Codex 会装一个 **Apple 授权插件（authorization plug-in）**，参与 macOS 的解锁流程，在受信任的电脑操控回合里**临时解锁** Mac、同时盖住所有显示器画面、保留锁屏保护。它的安全护栏是：授权窗口短时效、只对活跃回合开放、**一旦检测到本地有人动键盘 / 鼠标就立刻重新锁屏**并暂停自动解锁。
