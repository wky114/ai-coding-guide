...
> 📚 **系列导航**：上一篇 [05 · 接入 DeepSeek 等国产模型](05-third-party-models.md) 把「换大脑」那条野路子讲透了。配置篇到此为止——这一篇正式上手干活，**让 Codex 实打实改一次你的代码，跑通从提需求到批准落地的完整闭环**。下一篇 [07 · 桌面 App 全景](07-desktop-app.md) 再把桌面端那张脸看个遍。
...
说句实话，这篇我最想塞给你的，就是这个动作。今天咱们不碰大项目，**建个三行代码的玩具，五分钟走一遍「提需求 → Codex 分析 → 沙箱里改 → 看 diff → 批准 / 拒绝」的完整一圈**，让你第一次拿到「Codex 帮我改对了代码、而且全程我看得明明白白」的成功体验。桌面 App 和 CLI 两条路我都给你走一遍。
...
为什么先聊这个？因为 Codex 不是许愿池——你扔进去什么质量的需求，它就回你什么质量的活。[02 核心概念](02-core-concepts.md) 讲过它是个**代理（Agent）**，干活靠一个循环：官方原话是它「调用模型，然后执行模型指示的动作，比如读文件、改文件、调工具，直到任务完成或被你取消」。翻成大白话还是那三个动作——**想 → 做 → 看**。
...
这时候**最关键的一幕来了**——Codex **不会**直接把你的文件改掉。它的默认节奏是（[02](02-core-concepts.md) 讲的沙箱 + 审批联手）：

1. 找到该改的文件（这里是 `main.py`）
2. **把它打算改成什么样，以 diff（差异对比）的形式摆给你看**
3. **停下来，等你点头**——动手改文件这一步要「出圈」，审批拦着先问你
4. 你批准了，它才真正落盘

**类比：同事拿着改好的稿子来找你过目。** 一个靠谱的同事，不会未经你同意就把共享文档改了——他会拿着修订版过来：「头儿，我想这么改，你瞅瞅行不行？」你点头他才合并。**Codex 默认就是这种「先给修订版、等你过目」的同事，而那份修订版就是 diff。**
...
三个都「是」，放心批准；有一个「不对」，**就该拒绝、补话**（下一节讲怎么补）。**这一遍扫描，就是「审 diff」**——前后不到十秒，却是新手和老手最大的分水岭。

### 批准还是拒绝，你的选项

看懂 diff 后，你会看到一个让你选的提示。不同入口文案略有出入，但意思无非这几种：

| 你的选择 | 效果 | 什么时候用 |
|---|---|---|
| **同意 / Yes** | 应用这一次改动 | 看懂了 diff，确认没问题 |
| **同意，且本次不再问** | 之后的改动自动应用 | 你已经信任它了，想提速 |
| **拒绝 / No，并说明原因** | 不改，你补充要求 | diff 里有你不想要的东西 |

**第一次，老老实实选「同意」就好，别急着开「不再问」那档。** 这话我用一小时的代价换来的：刚上手最容易图省事，一上来就把审批放到最松，结果它在你没细看的情况下连改五六个文件，**等你反应过来，已经分不清哪个改动是你要的、哪个是它自作主张加的了**。教训就一条：**越是不熟，越要让它一步一停。**

> ⚠️ Codex 默认带审批、改文件前先停下来问你，这是它的常态；但审批策略和沙箱模式是可以调松的（[02](02-core-concepts.md) 讲过 `read-only` / `workspace-write` 和 `on-request` / `never`）。一旦你把它调到「不问」那种全自动档，**这个 diff 关卡可能就不弹了**——所以放开权限前，先确认你真的信得过当前这摊活。

> 💡 **一句话总结**：Codex 改文件前**永远先给你 diff、等你点头**；审 diff 就扫三件事——改对地方没、逻辑认不认、删错东西没；**第一次老实点同意，别图快开全自动**。
...
Codex 在一个**线程（Thread）**里干活——官方定义：一个线程就是一次会话，你的提问加上后续的模型输出和工具调用，**一个线程里可以有好多轮对话**。所以你拒绝它的 diff 之后，这次会话的上下文还在，你直接补一句，它就顺着改。

**类比：点菜上来不合口，你不用换家餐厅，跟服务员说一声「这个太咸了，重新做一份淡点的」就行。** 整桌菜的语境、你之前的要求，厨房都还记着，改的只是你不满意那一点。拒绝 diff 然后补话，就是这个「退回去重做」的动作，成本极低。
...
> 💡 **一句话总结**：拒绝 diff 不是终点，而是「退回去重做」——**在同一个线程里补一句话说清哪不对，Codex 顺着改**，不用退出、不用从头解释。
...
这是 Codex 官方在快速上手里**反复强调**的一条，原话：「Codex 会改你的代码库，**建议在每个任务前后各创建一个 Git 检查点**，这样需要时能轻松回退。」注意——它讲的后悔药就是 **Git**，不是什么工具内置的撤销。
...
**第四步：让它改代码，然后审 diff、批准**

```text
给 main.py 里的 add 函数加上类型注解，并补充基本的错误处理
```

**预期**：Codex 给出一段 diff（红 `-` 旧行、绿 `+` 新行），然后**停下来等你选**。按第 04 节那三问扫一遍 diff，看懂后选「同意 / Yes」。
...
> 💡 **一句话总结**：CLI 全流程五步——建项目存档 → `codex` 启动 → 解释代码试水 → 改代码并审 diff 批准 → 回终端用 `cat` / `git diff` 验收；**记得在项目目录里启动，记得改前先 Git 存档**。
...
App 的整体节奏和 CLI 一模一样，**就是把「敲命令」换成了「点界面」，核心那个「审 diff → 批准」一步不少**。按官方快速上手的步骤走：
...
3. **确认选的是 Local**——官方特意强调：发第一条消息前，**确保左下角选的是 Local**，让 Codex 在你本机干活，而不是云端。这一步新手最容易忽略。
...
**预期**：App 里会弹出一个图形化的 diff 面板，一行行高亮给你看它要改什么。**这正是 App 比 CLI 舒服的地方——diff 看得更清楚、点接受 / 拒绝就是按钮**。审完点接受，改动落地。

> App 同样建议你**改前后各打一个 Git 检查点**（官方原话）；App 内置了 Git 功能，你也可以在界面里直接提交。
...
| **核心流程** | 提需求 → 审 diff → 批准 | **完全一样**，只是换成点界面 |

我自己的用法：**写代码、跑命令我常驻 CLI**（顺手、跨平台）；但遇到改动多、diff 长的活，我会切到 **App 里审 diff**，那个图形面板一行行看，比在终端里翻文本舒服不少。**一次登录后两端一般都能用（具体以官方鉴权说明为准），挑你顺手的。**

> 💡 **一句话总结**：桌面 App 把同样的活做成点点点（仅 Mac / Windows），**发消息前记得选 Local**；它的图形 diff 面板看长改动更舒服，但「审 diff 再批准」这个核心动作，和 CLI 一模一样。
...
flowchart TD
    ...
    C --> D[提需求: 改代码]
    D --> E[Codex 分析<br/>沙箱里生成 diff]
    E --> F{审 diff<br/>改对没·认不认·删错没}
    F -->|看懂了| G[批准 → 改动落地]
    F -->|有问题| H[拒绝 + 补一句话]
    H --> E
    G --> I[git diff 验收<br/>炸了 git restore 兜底]
```

这张图最该盯住的是中间那个**审 diff 岔口**：Codex 在沙箱里把改动生成 diff，**你看懂了才批准、看着不对就拒绝补话让它重来**——这个「人来把关」的环节，就是你和 Codex 协作的安全阀。开头我那一小时，丢的就是这一步。

> 💡 **一句话总结**：整个第一个任务，核心就一个闭环——**提需求 → Codex 沙箱里改 → 你审 diff → 批准或拒绝**；把「审 diff」这一步焊死在流程里，你就再不会重蹈我那场翻车。

---

## 10 小结

这一篇你干了一件实事：**完整跑通了 Codex 的第一个任务——从建项目、启动，到解释代码、改代码、审 diff 并批准落地**，CLI 和 App 两条路都走了一遍。
...
| 改代码 | 「给函数加类型注解……」 | 它**先给 diff、等你点头**才动手 |
| **审 diff** | 扫三问：改对没·认不认·删错没 | **新手和老手的分水岭，别跳过** |
| 反悔 | 「改回去」/ `git restore .` | Git 是最硬的后悔药 |

**你现在应该能**：在 CLI 或 App 里启动 Codex、用自然语言让它读懂并修改一段代码、**看懂它给的 diff 并决定批准还是拒绝**、知道改错了怎么退回去。

这套动作——**提需求 → 看 diff → 批准 / 拒绝 / 反悔**——就是你之后所有 Codex 使用的最小内核。后面再花哨的功能（MCP、子代理、技能、自动化……），本质都是在这个循环上做加法。**而那个最该刻进肌肉记忆的动作，永远是「审 diff」**——别像开头的我，用一小时去换这个教训。
