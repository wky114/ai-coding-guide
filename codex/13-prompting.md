...
不会拆的时候，Codex 自带两个帮你拆的档位，[07](07-desktop-app.md)、[09](09-ide.md) 里提过，这里说清它俩的分工：

- **`/plan`（计划模式）**：让 Codex 先探索、提方案，**批准前它只出方案、不直接动手改实现**。适合「我也吃不准这活儿该怎么动」的时候——先让它把步骤捋出来，你看着顺了再放行。
