...
新手最容易犯一个挺典型的错：把含数据库密码的配置直接写进了 `.claude/settings.json`，然后 `git push` 上去了。**等想起来的时候，密钥已经躺在仓库历史里了**，只能改密码、重写历史，折腾大半小时。其实这种东西本该放 `settings.local.json`——那个文件 Claude Code 默认就帮你 gitignore 了。
...
这个区别很容易吃亏：经常有人纳闷「用户级明明 deny 了某个命令，为什么项目里还能跑」——其实是因为，**权限规则是合并的不是覆盖的**，项目级 allow 了，和用户级的规则叠在一起评估。所以**权限别指望靠用户级「一刀切禁掉」**，得搞清楚合并规则。
