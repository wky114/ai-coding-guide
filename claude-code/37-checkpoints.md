...
这个「可配置」具体是哪个开关？是 settings.json 里的 `cleanupPeriodDays`（第 31 篇专门讲过 settings.json 的用户级 / 项目级配置）。官方 `settings.md` 说它默认 **20** 天清理不活跃会话的记录、最少 1 天，设成 `0` 会被直接拒绝——这个值同时管着检查点快照的留存。**对小白的建议：别动它。** 默认值够用了，你真正需要长期保存的进度，本来就该交给 git（下面第 05 节细说），而不是指望检查点替你存半年。
...
怎么当场分清它走的是哪种？**看它在干活时打出来的工具调用名**（第 03 篇讲过 `Ctrl+O` 能展开详细转录）：
...
它会基于当前会话**叉出一条新分支**去试，原会话完好。这招你先知道有，真到「想并行试两种思路」时再翻第 19 篇的会话管理。
