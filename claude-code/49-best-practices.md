...
最受用的是第三招「**参照现有模式**」。比如直接让它「加个导出 CSV 的功能」，它很可能整出一套全新的写法，跟项目里已有的导出逻辑风格完全不搭，返工对齐能花掉小半天。换成一句模板就好办了——「**先看 `XXX.ts` 里现有的导出是怎么做的，照那个模式来，别引新库**」。就加这么半句，它写出来的代码风格、用的工具函数全跟项目对齐，基本不用返工。**指一个现成的好例子给它，胜过你描述十句「我想要的风格」。**
...
这条特别值得记住。设想跟它死磕一个状态管理的 bug，纠正了估计有五六次，越改越乱，最后那版代码连自己都看不懂了。这时候一咬牙 `/rewind` 回最初状态、`/clear`，重开一个会话，把「这 bug 跟组件卸载时的异步回调有关，别动渲染逻辑」一次性说清——**新会话两轮就能修好**。由此可以给自己定条硬规矩：**同一个问题纠正满三次，无条件 `/clear` 重开**。
