...
> 📚 **系列导航**：上一篇 [31 settings.json：用户级 / 项目级配置] 把「配置写哪个文件、谁压谁」捋清楚了。这一篇接着讲一个就住在那套配置里的开关——**输出样式（output styles）**。它管的不是「Claude 知道什么」，而是「Claude 怎么回应你」。一行设置，就能让它从默认的「闷头干活的工程师」，切成边讲解边干、甚至根本不写代码的另一副样子。
...
举个最容易踩的场景：让一个完全不懂代码的人拿 Claude Code 改简历。开着默认的工程师人格去跟它对话，它动不动就想「帮我把这段拆成函数」「要不要加个测试」——**满脑子工程思维，跟改简历这事儿完全不在一个频道**。这种「让它别当工程师」的活，本来就该靠切换 output style 解决，而不是在 CLAUDE.md 里写一堆「请你忘掉你是程序员」。
...
| **Subagent**（第 23 篇） | 用**自己独立的**系统提示、模型、工具跑子代理 | 你要一个**单独作用域**的助手去干专注任务 |
