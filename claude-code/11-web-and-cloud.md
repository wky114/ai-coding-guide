...
设想这样一个场景：你在高铁上，临时收到一条消息——线上一个仓库的 README 里把安装命令写错了，得赶紧改。

**手边没带电脑，只有一台手机，外加那种时断时续的车厢 WiFi。**

放在以前，这就是「等到了再说」。但这时候掏出手机打开 claude.ai/code ，选中那个仓库，敲一句「修正 README 里的安装命令，npm 包名应该是 xxx」，把手机塞回兜里。**等出站的时候，一个改好的分支已经躺在 GitHub 上等你提 PR 了——全程大概 8 分钟，一行命令都没敲过，电脑全程没开。**
...
这意味着什么？默认情况下，**云端的 Claude 改完文件直接推分支，不会停下来一行行等你点同意**。所以派云端任务时，**任务描述一定要写清楚、写具体**——它不会中途回头问你「这样改行不行」。这个亏很容易吃：图省事派了个模糊的「优化一下这个模块」，回来一看它自作主张重构了一大片，**远不是你想要的那点小改动**。稳妥的做法是，云端任务一律写明白文件名、要改什么、预期行为。
