...
举个最能体感的场景：调一个前端的诡异 bug，页面加载时控制台报一个 `undefined`，盯着代码看了半天也没看出哪错了。接上 Chrome 后你只要说「打开 dashboard 页，把加载时控制台里的报错念给我听，再定位是哪段代码」——它读完报错、顺着调用栈找到了一个没做空值判断的地方，**前后不到两分钟**。换以前，你得自己开 F12、复制报错、再贴回终端描述一遍，光来回倒腾就比这久。
...
> Arc 这个坑很容易踩。日常浏览器要是 Arc（同样 Chromium 内核），想当然以为装上扩展就能用，结果 `/chrome` 死活连不上。翻文档才看到那行小字「尚不支持 Brave、Arc 或其他基于 Chromium 的浏览器」——**白折腾二十分钟**。老老实实开一个 Chrome 专门给它用，才能秒连。
