...
说出来有点反直觉，但很多人第一次想「并行干活」时，干的就是这么一件蠢事：开了两个终端，都 `cd` 到同一个项目目录，一个让 Claude 改前端登录页，另一个让它顺手修一个后端的 bug。

心里美滋滋地想：「这不双倍效率嘛。」

结果呢——两边都在改 `package.json`，一边刚写进去的依赖，被另一边的改动**直接覆盖回滚**了；`git status` 一看，工作区乱成一锅粥，谁改的哪行根本分不清。最后花了比「老老实实一件件干」多得多的时间，才把两边的改动手动捋开。**这时候才明白：并行不是「多开几个窗口」这么简单，核心是「别让它们踩到同一块地」。**
...
这个坑很容易栽进去：兴冲冲 `-w` 开了个会话准备改后端，结果它连不上数据库——折腾半天才反应过来，`.env` 压根没复制进 worktree。补上 `.worktreeinclude` 之后就再没这毛病了。
