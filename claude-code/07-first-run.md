...
> 📚 **系列导航**：上一篇 [06 Coding Plan：订阅套餐与计费] 帮你把钱的事算明白了。这一篇该真刀真枪上手了——从打开终端到跑通第一个任务，让你第一次「成功」。
...
> ⚠️ 万一卡在权限报错（Mac / Linux 上看到 `Error: EACCES: permission denied` 这类），通常是 `~/.claude` 目录的权限不对。菜鸟教程给的修法是把该目录的所有权改回当前用户：`sudo chown -R $(whoami) ~/.claude`。具体排查后面「常见问题」篇会细讲，这里先知道有这么个坑。
