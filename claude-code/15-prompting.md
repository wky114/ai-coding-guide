...
这比「项目里有个 user 类型文件，你去找找」靠谱一万倍。官方文档明确写了 `@` 引用「在响应前读取文件的完整内容」；还有个隐藏福利——**`@` 引用一个文件时，会顺带把它所在目录和父目录的 `CLAUDE.md` 也拉进上下文**，等于连带把项目规矩也喂给它了。
...
**进阶玩法：`/goal` 把验收标准变成「不达标不收工」。**（需要 Claude Code v2.1.139 以上）普通提问里写验收标准，是「这一轮跑一下」；而 `/goal` 是把标准**钉成整个会话的目标**——每跑完一轮，一个小模型（默认 Haiku）会按你的条件复查一遍，**没达成就自动开下一轮，不把控制权还给你**，直到条件满足才停。
