...
- 这能力**能被关掉**：设置里有个 `disableSkillShellExecution`，设成 `true` 后这些命令一律不执行、替换成一句「已被策略禁用」。在团队的托管设置里它最有用——**防止有人在共享仓库的命令里塞个乱跑的 shell 命令**。
...
| 想塞长篇参考资料又不占上下文 | ❌ 内容写死、调用即全进 | ✅ 渐进式披露，用到才加载全文 |
