...
**Remotion 这个开源项目，在 GitHub 上攒了两万多颗 star。** 一个「用 React 写视频」的框架，能有这个量级的关注度，本身就说明一件事——**「把视频当代码来写」不是小众极客的自娱自乐，是真有人靠它干活**。
...
runoob 那篇教程有句话很在理：**用 `spring()` 做的弹性动画，比线性动画自然 10 倍。** 拿前面那个发布预告来说，一开始 Logo 是「啪」一下硬切到中间的，丑得很；后来跟 Claude 说「让 Logo 用弹性效果弹进来」，它把 `interpolate` 换成了 `spring`，Logo 就有了那种「弹一下稳住」的高级感。**所以你想让动画显得专业，提需求时直接说「用弹性缓动」**——这是一条很实用的经验。
