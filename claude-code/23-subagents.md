...
**`@` 点名**——输入 `@` 从弹出的列表里选，**这种是「保证」用它**，不把选择权留给 Claude：

```text
@code-reviewer 看看这次认证相关的改动
```
...
进去后，**用 `@` 直接点名**（保证用它，别让 Claude 自作主张）：

```text
@code-reviewer 点评一下 calc.py
```
...
> ⚠️ 如果敲 `@code-reviewer` 时列表里没有它，多半是**子代理文件加载没生效**——手写的文件需要重启会话（第 04 节那个坑）。退出重进，或者干脆用 `/agents` 界面建（免重启）。
