...
这是一段很容易在工位旁听到的对话。说白了，**让人惊讶的点在于「一个人怎么同时指挥多个 AI」**，而这正是这一篇要拆的东西——Agent teams（智能体团队），让多个 Claude Code 会话像一个团队那样协作，你只负责分派和验收。
...
**从「研究 / 审查」这类不写代码的活开始。** 审 PR、调研一个库、排查 bug——这些任务边界清楚、不写代码，**能让你体会并行探索的价值，又不带来并行写代码那一堆协调麻烦**。比如第一次用 agent teams 就拿来审一个 PR，三个队友分别盯安全、性能、测试覆盖，体验顺滑，这才好往「并行改代码」上推。
