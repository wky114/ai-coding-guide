...
**预期**：Claude **第一次调这个 server 会停下来问你要不要批准**（就是第 05 节 checkpoint 那族里、工具首次调用要批准这一闸）——批准它。然后它返回这两个术语的官方说明，**而且输出里那个工具调用旁边会标着 `claude-code-docs`**。看到这个标记 = 答案是真从官方文档查来的，不是模型记忆里的。
