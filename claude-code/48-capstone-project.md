...
小任务你可以一处一处批准。但中等项目要跑好几个会话、改十几个文件，**「第十次批准时你已经不是在审查，只是在无脑点过」**——官方原话。所以开工就该把权限基线定好。三条路，看场景挑：
...
| 退回档位 | 用什么 | 退到哪 | 适合 |
|---------|--------|--------|------|
| **轻档：撤刚才的编辑** | 检查点，`/rewind` 或双击 `Esc` | Claude 改这一批文件之前 | 改岔一两处、刚发现就想撤 |
| **重档：回到某个干净提交** | git，`git restore .` / `git reset --hard` | 你之前 commit 过的某个稳定点 | 方向整个偏了、想从某节点重来 |
