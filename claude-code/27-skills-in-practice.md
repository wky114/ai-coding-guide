...
**`/skills` 菜单**——在输入框敲 `/skills` 回车，会弹出一个可视化菜单，把所有 skill 列出来，还能看到每个是开是关。比纯文字列表更直观。
...
**这里有个上一篇讲过、但你现在该亲眼对上号的关键点**：那行 `description`，就是 Claude 判断「这次该不该用这个 skill」的唯一依据。
...
这里有个**官方明确的安全细节，务必记住**：别人项目里检入的 skill，**第一次用会弹一个「工作区信任」对话框让你确认**。为啥要这一道？因为 skill 可以给自己授予工具权限（比如自动跑命令），**信任一个仓库前，先扫一眼它的 skill 里写了啥**，别闭眼点同意。官方原话：
