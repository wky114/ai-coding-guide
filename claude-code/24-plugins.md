...
说句实话，**单人单项目，插件的好处你感受不深；一旦人多了、项目多了，散装就开始拖后腿**。带团队之后往往就会彻底转向插件——光是「新人入职配环境」这一项，就能从半小时口头教学压到一条命令。
...
说白了：**`.claude-plugin/` 这个文件夹里只准放 `plugin.json` 一个东西**，其他组件文件夹全部摆在插件根目录下，跟 `.claude-plugin/` 平级。这个坑第一次打包时很容易踩——一旦把 `skills/` 塞进了 `.claude-plugin/`，插件会加载但 skill 死活不出现，排查半天才发现是位置摆错。
