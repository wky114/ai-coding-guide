...
> 📚 **系列导航**：上一篇 [49 最佳实践] 把「该这么用」的正面打法摊开讲了一遍。这一篇翻到背面——**专挑「不该这么用」的坑**。同一个工具，有人用得飞起，有人用成累赘，差别往往不在「会不会高级功能」，而在**有没有踩进那几个最常见的反模式**。这一篇我把它们一个个拎出来示众，每个都配一句「正确该怎么做」。
...
- **日常迭代，想少点打断**：用 `acceptEdits`——文件编辑不问，但危险的 shell 命令、超出工作目录的写入**照样停下来问你**。官方管这叫「日常开发的甜点区」，也是平时用得最多的一档。
